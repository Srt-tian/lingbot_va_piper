...
class Trainer:
    def __init__(self, config):
        if config.enable_wandb and config.rank == 0:
            ...
            self.wandb.init(
                ...
                project=getattr(config, "wandb_project", os.getenv("WANDB_PROJECT", "lingbot_va")),
                # dir=log_dir,
                config=config,
                mode="online",
                name=getattr(config, "wandb_name", os.getenv("WANDB_NAME", os.path.basename(os.path.normpath(config.save_root))))
        ...
        if getattr(config, "disable_activation_checkpoint", False):
            logger.info("Activation checkpointing disabled for smoke test.")
        else:
            logger.info("Setting up activation checkpointing ...")
            apply_ac(self.transformer)

        if getattr(config, "disable_fsdp", False):
            logger.info("FSDP disabled for smoke test; moving transformer to single GPU...")
        ...
        if dist.is_initialized():
            dist.barrier()
        if config.rank == 0:
            logger.info("Dataset setup barrier passed on all ranks")
        dataset_init_worker = getattr(config, "dataset_init_worker", max(1, config.load_worker))
        logger.info(f"Building MultiLatentLeRobotDataset with dataset_init_worker={dataset_init_worker}, load_worker={config.load_worker}")
        train_dataset = MultiLatentLeRobotDataset(config=config, num_init_worker=dataset_init_worker)
        logger.info(f"Dataset built with len={len(train_dataset)}")
        if dist.is_initialized():
            dist.barrier()
        if config.rank == 0:
            logger.info("Dataset built barrier passed on all ranks")
    ...
    def _debug_log(self, message):
        if getattr(self.config, "rank", 0) == 0 and getattr(self.config, "debug_train_progress", False):
            logger.info(message)
    ...
    def train(self):
        ...
        while self.step < self.config.num_steps:
            ...
            self._debug_log(f"train loop step={self.step}: fetching batch")
            batch = self._get_next_batch()
            self._debug_log(f"train loop step={self.step}: batch fetched")
            
            self._debug_log(f"train loop step={self.step}: train step start")
            losses = self._train_step(batch, step_in_accumulation)
            self._debug_log(f"train loop step={self.step}: train step done")
...
def run(args):
    ...
    if os.getenv("MODEL_ROOT"):
        config.wan22_pretrained_model_name_or_path = os.environ["MODEL_ROOT"]
    if os.getenv("DATASET_ROOT"):
        config.dataset_path = os.environ["DATASET_ROOT"]
        config.empty_emb_path = os.path.join(config.dataset_path, "empty_emb.pt")
        info_path = os.path.join(config.dataset_path, "meta", "info.json")
    ...
    env_overrides = {
        "LINGBOT_BATCH_SIZE": ("batch_size", int),
        "LINGBOT_GRAD_ACCUM": ("gradient_accumulation_steps", int),
        "LINGBOT_NUM_STEPS": ("num_steps", int),
        "LINGBOT_SAVE_INTERVAL": ("save_interval", int),
        "LINGBOT_LOAD_WORKER": ("load_worker", int),
        "LINGBOT_DATASET_INIT_WORKER": ("dataset_init_worker", int),
        "LINGBOT_LR": ("learning_rate", float),
    }
    for env_name, (cfg_name, caster) in env_overrides.items():
        if os.getenv(env_name) is not None:
            setattr(config, cfg_name, caster(os.environ[env_name]))
    config.debug_train_progress = os.getenv("LINGBOT_DEBUG_PROGRESS", "0").lower() in ("1", "true", "yes")
    if os.getenv("LINGBOT_DISABLE_FSDP") is not None:
        config.disable_fsdp = os.getenv("LINGBOT_DISABLE_FSDP", "0").lower() in ("1", "true", "yes")
    if os.getenv("LINGBOT_DISABLE_ACTIVATION_CHECKPOINT") is not None:
        config.disable_activation_checkpoint = os.getenv("LINGBOT_DISABLE_ACTIVATION_CHECKPOINT", "0").lower() in ("1", "true", "yes")
    ...
    if os.getenv("WANDB_API_KEY"):
        ...
        if os.getenv("WANDB_NAME"):
            config.wandb_name = os.getenv("WANDB_NAME")
    elif os.getenv("WANDB_DISABLED", "").lower() in ("1", "true", "yes"):
