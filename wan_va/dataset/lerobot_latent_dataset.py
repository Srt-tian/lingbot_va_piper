...
class LatentLeRobotDataset(LeRobotDataset):
    def __init__(
        ...
        try:
            episodes = self.episodes if self.episodes is not None else sorted(self.meta.episodes.keys())
            self.episodes = episodes
            data_file_paths = [self.meta.get_data_file_path(ep_idx) for ep_idx in episodes]
            assert all((self.root / fpath).is_file() for fpath in data_file_paths)
            self.hf_dataset = self.load_hf_dataset()
        except (AssertionError, FileNotFoundError, NotADirectoryError) as exc:
            if Path(self.repo_id).is_absolute():
                raise FileNotFoundError(f"Local LeRobot parquet files are incomplete under {self.root}") from exc
            self.revision = get_safe_version(self.repo_id, self.revision)
            self.download_episodes(download_videos)
            self.hf_dataset = self.load_hf_dataset()
        self.episode_data_index = get_episode_data_index(self.meta.episodes, self.episodes)
        self.episode_index_to_data_pos = {
            int(episode_index): pos for pos, episode_index in enumerate(self.episodes)
        }
        ...
        norm_path = Path(config.dataset_path) / "meta" / "lingbot_action_norm.json"
        if norm_path.is_file():
            with norm_path.open("r", encoding="utf-8") as norm_file:
                norm_stat = json.load(norm_file)
            self.norm_stat_source = str(norm_path)
        else:
            norm_stat = config.norm_stat
            self.norm_stat_source = "config.norm_stat"
        self.q01 = np.array(norm_stat['q01'], dtype='float')[None]
        self.q99 = np.array(norm_stat['q99'], dtype='float')[None]
    ...
    def parse_meta(self):
        out = []
        episode_ids = getattr(self.config, "episode_ids", None)
        episode_start = getattr(self.config, "episode_start", None)
        episode_end = getattr(self.config, "episode_end", None)
        episode_ids = set(episode_ids) if episode_ids is not None else None
        for key, value in self.meta.episodes.items():
            episode_index = value["episode_index"]
            if episode_ids is not None and episode_index not in episode_ids:
                continue
            if episode_start is not None and episode_index < episode_start:
                continue
            ...
            for acfg in action_config:
                ...
                latent_span = self._find_latent_span(
                    cur_meta["episode_index"], cur_meta["start_frame"], cur_meta["end_frame"],
                    allow_unique_fallback=len(action_config) == 1)
                if latent_span is None:
                    continue
                cur_meta["start_frame"], cur_meta["end_frame"] = latent_span
    ...
    def _find_latent_span(self, episode_index, start_frame=None, end_frame=None, allow_unique_fallback=False):
        ...
        for key in self.used_video_keys:
            ...
            spans = set()
            for latent_file in cur_path.glob(f"{prefix}*.pth"):
                stem = latent_file.stem
                try:
                    file_start, file_end = stem[len(prefix):].split("_", 1)
                    spans.add((int(file_start), int(file_end)))
                except ValueError:
                    continue
            shared_spans = spans if shared_spans is None else shared_spans & spans
        if not shared_spans:
            return None
        requested = (int(start_frame), int(end_frame)) if start_frame is not None and end_frame is not None else None
        if requested in shared_spans:
            return requested
        if allow_unique_fallback and len(shared_spans) == 1:
            return next(iter(shared_spans))
        raise ValueError(
            f"Episode {episode_index}: no unambiguous latent span for {requested}; "
            f"available={sorted(shared_spans)}"
        )
    ...
    def _get_global_idx(self, episode_index: int, local_index: int):
        data_pos = self.episode_index_to_data_pos[int(episode_index)]
        ep_start = self.episode_data_index["from"][data_pos]
    ...
    def _action_post_process(self, local_start_frame, local_end_frame, latent_frame_ids, action):
        ...
        expected_action_per_frame = int(frame_stride * 4)
        if self.config.action_per_frame != expected_action_per_frame:
            raise ValueError(
                f"action_per_frame={self.config.action_per_frame}, expected "
                f"{expected_action_per_frame} from latent frame stride {frame_stride}"
            )
